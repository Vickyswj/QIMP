...
Instance Mminus_proper(m n:nat): Proper (mat_equiv ==> mat_equiv ==> mat_equiv) (@Mminus m n).
Proof.
hnf;intros A C H1.
hnf;intros B D H2.
unfold mat_equiv,Mminus,get in *.
intros. rewrite H1,H2. reflexivity.
Qed. 
...
Proof.
  ...
  induction n.
  -  simpl. lca.
  -  simpl in *. specialize (IHn A B). rewrite IHn.
     +  f_equal.
          assert (n<S n) by lia.
          specialize (H1 (exist _ n H) (exist _ n H)).
...
Lemma Mmult_n_1_l : forall {n} (A : Square n),
  1 ⨉ A ≡ A.
Proof. intros. simpl. rewrite Mmult_1_r. reflexivity. Qed.
